...
import type { ChallengeConfig } from "@/app/actions/challenge-config";
import {
...
// ── Slot-indexed styling (index 0→amber, 1→rose, 2→emerald) ─────────────────

const MUSEUM_SLOT_STYLES = [
    {
...
const CAFE_TIER_STYLES = [
    { color: "from-slate-50 to-slate-100",   pointColor: "text-slate-600",   barColor: "bg-slate-300",   width: "33%",  highlight: false },
    { color: "from-amber-50 to-yellow-100",  pointColor: "text-amber-600",   barColor: "bg-amber-400",   width: "66%",  highlight: false },
    { color: "from-emerald-50 to-green-100", pointColor: "text-emerald-600", barColor: "bg-emerald-500", width: "100%", highlight: true  },
] as const;
...
// ── Fallback data (used when no active config) ────────────────────────────────

const FALLBACK_CONFIG = {
    badge_label: "이번 달",
    blog_title: "파리 미술관 여행 포스팅 챌린지",
    blog_subtitle: "4월 미션 콘텐츠에 파리 3대 미술관 중 1개 포스팅",
    blog_condition: "4월 미션 콘텐츠 (여행 준비·기대평, 여행지 정보 전달)에 키워드 3개 이상 자연스럽게 포함하여 포스팅 후 제출",
    blog_museums: [
        {
            name: "루브르 박물관",
            emoji: "🏛️",
...
function KeywordPill({
    ...
    if (mode === "select") {
        return (
            <button
                onClick={onSelect}
                className={cn(
                    "inline-flex items-center gap-1.5 px-3 py-2 min-h-[44px] rounded-full text-[11px] font-bold border-2 transition-all duration-200 active:scale-95",
                    selected
                        ? "bg-white shadow-sm scale-[1.02]"
                        : "bg-white/70 border-slate-200 text-slate-500 hover:border-slate-300 hover:bg-white"
                )}
                style={selected ? { color: accent, borderColor: accent } : undefined}
            >
                {selected && <Check className="w-2.5 h-2.5 shrink-0" strokeWidth={3} />}
                {word}
            </button>
        );
    }
    ...
    return (
        <button
            onClick={copy}
            className={cn(
                "inline-flex items-center gap-1.5 px-3 py-2 min-h-[44px] rounded-full text-[11px] font-bold border transition-all duration-200",
...
function SubmitPanel({
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            const result = await submitChallenge({ challengeType, postUrl: url, museum, rewardType, selectedKeywords });
...
function BlogChallengeCard({ title, subtitle, condition, museums }: {
    title: string;
    subtitle: string;
    condition: string;
    museums: MuseumWithStyle[];
}) {
            ...
            <div className="relative z-10 p-6 sm:p-8 lg:p-10 flex flex-col flex-1 gap-6 sm:gap-8">
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button
                            onClick={() => { setRewardType(rewardType === "points" ? null : "points"); setSelectedKeywords([]); setShowSubmit(false); }}
                            className={cn(
                        ...
                        <button
                            onClick={() => { setRewardType(rewardType === "naver_pay" ? null : "naver_pay"); setSelectedKeywords([]); setShowSubmit(false); }}
                            className={cn(
                                "relative flex flex-col gap-2 p-4 sm:p-5 rounded-brand border-2 text-left transition-all duration-300 min-h-[88px] sm:min-h-0",
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 sm:grid sm:grid-cols-3 scrollbar-none -mx-1 px-1">
                        {museums.map((m, idx) => (
                            <button
                                key={m.name}
                                onClick={() => { setActiveMuseum(activeMuseum === idx ? null : idx); setSelectedKeywords([]); setShowSubmit(false); }}
                                className={cn(
                ...
                {activeMuseum !== null && (
                    <div className="animate-in fade-in slide-in-from-top-2 duration-400 space-y-3">
                        ...
                        {(rewardType === "naver_pay" || showKeywords === activeMuseum) && (
                            <div className={cn(
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {museums[activeMuseum].keywords.map((kw) => (
                                        <KeywordPill
                                            key={kw}
                                            word={kw}
                                            accent={museums[activeMuseum].accent}
                                            mode={rewardType === "naver_pay" ? "select" : "copy"}
                                ...
                                {rewardType === "naver_pay" && selectedKeywords.length === 3 && (
                                    <div className="mt-3 flex items-center gap-2 text-[11px] font-black text-emerald-600 animate-in fade-in duration-300">
                                        <CheckCircle2 className="w-3.5 h-3.5" />
                                        3개 선택 완료 — 이제 링크를 제출할 수 있습니다
                                    </div>
                                )}
                            </div>
                        )}
                ...
                <div className="flex gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-100">
                    ...
                    <div className="space-y-0.5">
                        ...
                        <p className="text-[11px] font-bold text-slate-400 leading-relaxed">{condition}</p>
                    </div>
                </div>
                        ...
                        >
                            ...
                            {activeMuseum === null
                                ...
                                : rewardType === null
                                    ? "리워드를 선택해 주세요"
                                    : rewardType === "naver_pay" && selectedKeywords.length < 3
                                        ? `키워드 ${selectedKeywords.length}/3개 선택 필요`
                                        : "챌린지 링크 제출하기"}
                        </Button>
                    ) : (
                        <SubmitPanel
                            challengeType="blog_paris"
                            museum={activeMuseum !== null ? museums[activeMuseum].name : undefined}
...
function CafeChallengeCard({ dateRange, subtitle, tiers }: {
    dateRange: string;
    subtitle: string;
    tiers: CafeTierWithStyle[];
}) {
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-bold text-slate-400 mt-1">
                                <span className="text-indigo-500 font-black">{dateRange}</span> · {subtitle}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    {tiers.map((tier) => (
                        ...
                        >
                            ...
                            <span className={cn("relative text-2xl font-black", tier.pointColor)}>
                                {tier.highlight ? "💸💸💸" : tier.days === tiers[1]?.days ? "💸💸" : "💸"}
                            </span>
                        </div>
                    ))}
                </div>
...
/** "2026-05" → "5.1 — 5.31",  "2026-06" → "6.1 — 6.30" */
function computeCafeDateRange(yearMonth: string): string {
...
export default function ChallengeContent({ config }: { config: ChallengeConfig | null }) {
    const c = config ?? FALLBACK_CONFIG;

    // Always compute date range from the month string — never rely on the stored string
    const dateRange = computeCafeDateRange(
        "month" in c ? (c as ChallengeConfig).month : currentYearMonth()
    );

    // Merge DB museums with slot-indexed styles (max 3 slots)
    const museums: MuseumWithStyle[] = c.blog_museums.slice(0, 3).map((m, idx) => ({
        ...m,
        ...MUSEUM_SLOT_STYLES[idx],
    }));

    // Merge DB cafe tiers with slot-indexed styles (max 3 tiers)
    const tiers: CafeTierWithStyle[] = c.cafe_tiers.slice(0, 3).map((t, idx) => ({
        ...t,
        ...CAFE_TIER_STYLES[idx],
    }));
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-10 lg:px-10 lg:py-16 space-y-6 sm:space-y-8 animate-in fade-in duration-700 pb-32">
            ...
            <div className="grid grid-cols-1 gap-5 sm:gap-6">
                <BlogChallengeCard
                    ...
                    condition={c.blog_condition}
                    museums={museums}
                />
                <CafeChallengeCard
                    dateRange={dateRange}
                    subtitle={c.cafe_subtitle}
                    tiers={tiers}
                />
            </div>
